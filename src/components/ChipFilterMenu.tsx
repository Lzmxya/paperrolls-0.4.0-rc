...
import { ChipFilterProps, Icon } from "@/components";

interface ChipFilterMenuProps extends Partial<ChipFilterProps> {
  children: ReactNode;
}

export function ChipFilterMenu({ children, isOn, label }: ChipFilterMenuProps) {
  return (
    <Popover className="relative flex">
      <Popover.Button as="div">
        ...
        >
          <Icon
            name="check"
            className={`overflow-hidden transition-all ${isOn ? "w-6" : "w-0"}`}
          />
